    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 401) {
                console.warn("[apiService] Token inválido ou expirado (Erro 401). Redirecionando para o login.");
                // Usa o authService para um logout limpo, se disponível
            ...
            throw new Error(errorData.description || errorData.detail || errorData.message || `Erro: ${response.status}`);
        }
        ...
        console.error(`[apiService] Erro na chamada para ${endpoint}:`, error);
